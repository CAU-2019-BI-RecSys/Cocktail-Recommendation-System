...
export default {
    ...
    methods:{
        ...
        getRecommendInfo(){
            this.loading = true
            let currentObj = this
            currentObj.axios.post('http://localhost:8000/api/cocktail/recommend/CF/',{
            ...
            currentObj.axios.post('http://localhost:8000/api/cocktail/recommend/CBF/',{
                like_list : currentObj.likeCocktails,
                count : 10
            })
            .then(function(response){
            ...
            setTimeout(function(){
                ...
                for(let [index] in currentObj.recommendedInfosCFUser){
                    recInfos.push(currentObj.recommendedInfosCFUser[index])
                }
                for(let [index] in currentObj.recommendedInfosCFItem){
                    recInfos.push(currentObj.recommendedInfosCFItem[index])
                }
                console.log("rec infos")
                console.log(recInfos)
                if(recInfos.length == 0){
                    console.log("length 0")
                    recInfos = currentObj.recommendedInfosCBF
                }
                else if(recInfos.length > 0 && recInfos.length < 20){
                    console.log("length 1~20")
                    currentObj.axios.post('http://localhost:8000/api/cocktail/recommend/CBF/',{
                        like_list : recInfos,
                        count : 20 - recInfos.length
                    })
                    .then(function(response){
